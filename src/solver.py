...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(self.train_config.n_epoch):
            ...
            with tqdm(self.train_data_loader, desc=f"Epoch {e + 1}/{self.train_config.n_epoch}", ncols=100) as tbar:
                for batch in tbar:
                    ...
                    if self.train_config.data == "ur_funny":
                        ...
                        y = y.mean(dim=1, keepdim=True)  # (batch, 1)
                        y_tilde = y_tilde.view(-1, 1)  # (batch, 1)
            ...
            self.writer.add_scalar("Loss/valid", valid_loss, e)
        ...
        self.writer.close()  # === 학습 종료 후 writer 닫기 ===
    ...
    def calc_metrics(self, y_true, y_pred, mode=None, to_print=False):
        if self.train_config.data == "ur_funny":
            ...
            if len(test_truth.shape) == 2 and test_truth.shape[1] == 7:
                test_truth = np.mean(test_truth, axis=1)
            if len(test_preds.shape) == 2 and test_preds.shape[1] == 7:
